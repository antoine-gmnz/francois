...
- 2026-08-20 · api · Two verbs that mutate the same thing at different scopes never coexist — delete the broader one rather than leave it dormant — because `git add -A` beside `commit(paths)` gave one action two truths and showed neither · diff-review
- 2026-08-20 · ui · A view rendering a historical ref is a SNAPSHOT: a change event refreshes the chrome around it and never its body — because content moving under a reader who deliberately chose a fixed point is worse than being briefly stale · diff-review
- 2026-08-20 · ui · State a view INFERS from the user's scroll position may annotate a row but must never resize, collapse or remove it — because the inference then moves the document under the very gesture that produced it · diff-review
