...
- **FR-17 (event contract).** Every summary recomputation triggered by a **change** — the fs watcher (FR-15), a `tool.done` (FR-16), or a successful `commit` (FR-25, whose `.git/`-only writes the watcher deliberately ignores) — results in exactly one broadcast on `francois:diff:event` with `{ type: 'diff.changed', sessionId, fileCount }`, where `fileCount = files.length` from that computation. **`getSummary` itself does NOT broadcast** (amended — it did until v0.37): a read is not a change, and the echo it emitted was a feedback loop as soon as a second listener refetched on `diff.changed`. The roster's fleet sync re-reads the summary of every settled session on every event, so its own read echoed back into itself, and with the DIFF tab open it ping-ponged with `DiffView`'s subscription — every idle session spinning a permanent `git status` + `git diff --numstat` loop, which is what made the app lag in proportion to the number of open sessions. There is still no separate "refresh" IPC channel; `getSummary` is idempotent and serves both automatic and user-initiated refreshes, and the event channel stays authoritative for anyone (e.g. `app-shell`'s badge) that isn't also holding the full summary — seeded by its own `getSummary` on selection and kept live by the change-triggered broadcasts above.
...
- **FR-22 (stage hotkey).** `s` runs `stageAll` when the DIFF tab is visible and no text input anywhere in the app has focus. It is a no-op (key is ignored, no IPC call) when `files.length === 0` or the session is `NOT_A_GIT_REPO` or a stage/commit mutation or summary reload is in flight. The **per-file `getFileDiff` fetch deliberately does not gate it**: it fires on every file click, and gating on it made the footer hints go inert (and stay inert if that fetch never settled) just from clicking through the file list — staging is unaffected by which file's diff is on screen.
- **FR-23 (commit hotkey — open).** `c` opens the inline commit bar when the DIFF tab is visible and no text input has focus. `c`'s inert conditions are narrower than `stageAll`'s (FR-22): it is inert only when the session is `NOT_A_GIT_REPO` or a stage/commit mutation or summary reload is in flight (not the per-file `getFileDiff`, per FR-22) — not when `files.length === 0`, because staged content can exist even with an empty/clean summary (e.g. the user staged everything via `s` and the working tree is otherwise clean, or staged specific files from the SHELL tab). A genuine "nothing staged" attempt is reported inline once `commit` is actually invoked, per FR-11/FR-26.
