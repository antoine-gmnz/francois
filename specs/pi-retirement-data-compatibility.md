...
- **FR-5** Reject Pi execution/mutation with RUNTIME_UNSUPPORTED before process spawn, probe, native-history/credential access, worktree creation or registry write. Includes create/send/submit/queue/control/model/settings/reconnect/newFrom; Pi account trust/setup/refresh/default/rename and Pi profile create/update/copy/remove. List/local transcript/metadata reads remain allowed. Unresolved identifiers retain existing missing-id errors. *Amended 2026-09-22:* removing a retired Pi account row is allowed — it is the only way out of a registry that still names it as the default. Only the row and its raw copy go; its directory (the user's own `~/.pi`) is never deleted and Pi sessions keep their `accountId`.
