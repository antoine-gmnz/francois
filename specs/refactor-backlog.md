...
- [ ] MEDIUM · src-tauri/src/process_util.rs:654 · quality · own_process_group/kill_tree are no-ops for tree cleanup on Windows (Child::kill only, no job object), so a Pi child's own grandchildren can be orphaned on Windows despite FR-7/Edge-cases §7 requiring Windows/WSL cleanup — wire a Windows job object into process_util (shared pre-existing gap with extensions::provider::kill_group) · deferred:pi-rpc-sessions
- [ ] MEDIUM · src-tauri/src/session/adapter/pi/process.rs:1332 · test-coverage · Acceptance §9 "Configured test extensions never execute under baseline policy (FR-10)" has no test proving a configured global/project extension package cannot defeat `--no-extensions`/`--no-approve` — only `baseline_argv_always_locks_extensions_and_approval_off` asserts the argv strings are present. Add a test that builds a `RuntimeConnectContext`/project config carrying a configured extension package and asserts the resulting argv/launch policy still excludes it · deferred:pi-rpc-sessions
- [ ] MEDIUM · src-tauri/src/session/adapter/pi/dispatcher.rs:955 · test-coverage · Acceptance §9 "Fake child covers fragmented UTF-8, interleaved replies, duplicates and unknown events (FR-2–3)" names interleaved replies, but every fake-child test drives request→response strictly one at a time — no test proves an out-of-order reply or an event landing between a command and its own response is routed to the right pending entry. Add a test with two concurrent dispatch/send calls whose responses arrive out of send-order (plus an event line between a command and its reply) and assert each completes the right pending entry · deferred:pi-rpc-sessions
...
## deferred:response-mode — SHIP-round leftovers (review round 1)

- [ ] MEDIUM · src-tauri/src/session/commands/turn.rs:421 · quality · `clear_full_reset_on_idle_session` does not assert `response_mode_sent` resets to `None` on `/clear`, leaving FR-10's requirement unverified (implemented at turn.rs:362) — seed `s.response_mode_sent = Some(ResponseMode::Concise)` before `apply_clear`, then assert it reads `None` · deferred:response-mode
- [ ] LOW · src-tauri/src/session/adapter/codex/runner.rs:365 · quality · FR-12's per-runtime prompt-bytes assertion is missing for codex and grok (mirrored at grok/runner.rs:365) — only `prefix_for`/`prefixed_prompt` are covered; the `begin_turn` wiring that calls `pending_prefix`/`mark_sent` and feeds the prefixed string into the stdin write (codex) / `-p` argv (grok) is untested — add a unit test or a testable seam around prompt composition, mirroring `claude_code::turn_args` and `openai::blocks::build_request_messages` · deferred:response-mode
- [ ] LOW · src/features/sessions/RunChip.tsx:132 · quality · FR-14 says picking a Response row leaves the panel open "matching the model and permission sections", but the pre-existing `pickMode` for permissions calls `setOpen(false)` on success — spec text and pre-existing permission behavior disagree; response-mode rows correctly follow the model/effort pattern — reconcile FR-14's wording next time `session-permission-mode` or this spec is touched · deferred:response-mode

## deferred:session-settings-sheet

- [ ] LOW · src-tauri/src/session/commands/lifecycle.rs:296 · quality · `session_update_settings` and every sibling switch verb (`session_switch_model`, `session_switch_effort`, `session_switch_permission_mode`) can never produce the `INTERNAL` error code their contract documents for "persist fails after the in-memory write", because `session::persistence::persist()` returns `()` not `Result` — thread a `Result` through `persist()` so a write failure surfaces as `INTERNAL` instead of being silently swallowed · deferred:session-settings-sheet
...
- [ ] LOW · src-tauri/src/session/adapter/pi/probe.rs:180-197 · quality · probe_wsl: report a distro-specific reason when the named WSL distro does not exist instead of generic Missing/RUNTIME_UNAVAILABLE · deferred:pi-runtime-distribution
- [ ] MEDIUM · src-tauri/src/session/adapter/pi/discovery.rs:161-175 (`evaluate_version`) · spec-follow-up · FR-4/FR-5 as originally worded asked for a version match to also pass the RPC protocol probe (`get_state`, `prompt`, …) before certification; this round only compares `--version` banner text (`provenance` never reaches `certified`), and FR-5 was amended (round 1 fix) to scope this feature to version-string certification only, per the audit's "no real Pi runtime … was executed" finding (no captured RPC transcript exists to probe against). → **Fix (future feature, once a real Pi install can be audited)**: spawn Pi's actual RPC transport, drive `get_state`/`get_available_models`/`get_commands`/`get_entries`/`get_session_stats`/`prompt`/`steer`/`follow_up`/`clear_queue`/`abort`/`compact`/`set_model`/`get_available_thinking_levels`, capture `agent_settled`, diff against a checked-in fixture, and only then report `provenance: 'certified'` · deferred:pi-runtime-distribution
- [ ] LOW · src/features/accounts/cliTools.ts (`runtimeInstallProbeInput`) · spec-follow-up · FR-1/FR-8 expose a native/WSL `ClaudeRuntime` axis, but the frontend has no environment picker — `PiSetupCard` always probes `'native'` with `distro` unset (the design brief shows no picker). → **Fix (when the brief grows a picker or a WSL-only Pi user needs it)**: add the picker and thread `runtime`/`distro` through `runtimeInstallProbeInput` · deferred:pi-runtime-distribution
...
## deferred:pi-session-durability
- [ ] MEDIUM · src-tauri/src/session/adapter/pi/recovery.rs (`get_entries` call site) · spec-follow-up · FR-6: every reconnect re-fetches with `cursor: None`; no paging and no bounded streaming read of the owned native JSONL. A session too large for one RPC reply fails explicitly (never truncates) but cannot be rebuilt. → **Fix (once a real Pi capture fixes the cursor/paging semantics)**: page `get_entries` by the durable entry cursor, and fall back to a bounded streaming read behind the same private decoder · deferred:pi-session-durability
- [ ] MEDIUM · src-tauri/src/session/adapter/pi/recovery.rs + wire.rs · spec-follow-up · the `get_entries` reply shape, the native entry fields (`id`/`parentId`/`role`/`text`), the first-line `sessionId` identity check and what `--resume` expects (handshake `runtimeId` is used) are PROVISIONAL — no real Pi capture exists. Reconcile against a certified capture before Pi 10 enables the loop · deferred:pi-session-durability
- [ ] LOW · src-tauri/src/session/adapter/pi/recovery.rs · spec-follow-up · FR-7: after an APP crash a never-finalized partial assistant response is not recoverable (streaming deltas are never persisted, for any runtime); a Pi-child crash still finalizes partial output as interrupted (pi-transcript-events FR-9) · deferred:pi-session-durability
- [ ] LOW · src-tauri/src/session/adapter/pi/recovery.rs (`validate_resume`) · quality · a moved cwd/worktree maps to RUNTIME_SESSION_CORRUPT with an actionable path message — the closed `RuntimeRecovery.state` vocabulary has no dedicated `moved` state · deferred:pi-session-durability
- [ ] LOW · src-tauri/src/session/adapter/pi/recovery.rs (`new_from_session`) · quality · `session_new_from` copies cwd verbatim but no `SessionWorktree`/`worktreeDistro` provenance (the spec names only cwd/project/account/profile/model), so removing the SOURCE worktree session can pull the directory out from under the new one · deferred:pi-session-durability
- [ ] LOW · src-tauri/src/session/adapter/pi/recovery.rs (version-transition backup) · complexity · hand-rolled 3-tuple semver parse duplicates `discovery.rs`'s private `parse_semver` — share one helper · deferred:pi-session-durability
- [ ] LOW · src-tauri/src/process_util.rs · quality · 999/1000 lines after `CommandBuilder::exact_env` — split by concern (spawn facade / PATH resolution / bounded run / tree kill) before the next change crosses the cap · deferred:pi-session-durability
- [ ] LOW · src/features/conversation/PiRecoveryBanner.tsx · spec-follow-up · "Create new session" does not carry the source session's composer draft into the new session (spec silent; the source's own draft is preserved) · deferred:pi-session-durability
