...
- [ ] LOW · src-tauri/src/session/adapter/openai/runner.rs · complexity · 1033 lines, over PIPELINE.md §Code layout's ~1000-line ceiling — split the pure decision + block-emission helpers (`round_trip_cap_hit`, `context_exceeded`, `tool_call_error`, `thread_tool_call`, `build_request_messages`, `emit_tool_block`/`finish_tool_block`/`buf_assistant_delta`/`finalize_open_text`) into a sibling `blocks.rs` · deferred:multi-provider-openai
- [ ] LOW · src-tauri/src/session/adapter/openai/skills.rs (`excludes_slash_command_entries`) · quality · FR-27's "excludes every kind: 'command' entry" is proven by a standalone manual `.filter()` assertion, not by running `build_skill_block` against a mixed list — removing the filter would not fail the test; extend the `FixedEnv` test to return one command + one skill and assert only the skill survives · deferred:multi-provider-openai
- [ ] LOW · src/ui/CapabilityNotice.tsx:20 / src/styles.css:1402 · quality · the design brief (specs/design/multi-provider-openai.md:97) specifies wrap-to-two-lines then middle-truncate with a title tooltip; the implementation end-clips via `-webkit-line-clamp: 2` (tooltip present, so full text stays reachable) — either middle-truncate via `middleTruncate` from accounts.ts, or amend the brief to accept end-clip-with-tooltip · deferred:multi-provider-openai
