...
- **FR-2 (append triggers — finalized only).** A block is written when it is finalized, never per streaming delta: **user** block on send (`buf_user`); **assistant** block on `assistant.done`; **tool** block on `tool.done` (so its `meta` is included); **subagent** block on its completion. An interrupted turn's open text block is finalized on teardown and so is written too — with the text streamed up to the interrupt, matching what the live view shows. The in-memory buffer carries a streaming assistant block earlier than this (from its first delta, so `getTranscript` is never missing an in-flight answer); only the **file** waits for the block to finalize. The `streaming` flag is never written (it reloads as `false`).
