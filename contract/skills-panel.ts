...
//   skills_run resolves the LISTED skill's exact `invocation` in the core and admits it
//     through the same internal admissions function as session_submit. It resolves
//     after ADMISSION (Result<void>); the receipt arrives on `queue.changed`. The
//     frontend never also calls session_submit for the same run. A name that is no
//     longer listed (it vanished on reconnect) is RUNTIME_UNSUPPORTED and the core
//     emits `skills.changed` so the listing refreshes.
