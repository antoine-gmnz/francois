...
fn run_shell_flusher(
    pending: &(Mutex<PendingOutput>, std::sync::Condvar),
    shared: &Mutex<Shared>,
...
fn run_shell_reader(
    ...
    let flusher = {
        ...
        std::thread::spawn(move || {
            run_shell_flusher(&pending, &shared, |data| {
                let _ = app.emit(
                    EVENT_CHANNEL,
                    ShellEvent::Data {
...
mod emission_tests {
    ...
    fn pending(data: &str) -> Arc<(Mutex<PendingOutput>, std::sync::Condvar)> {
        Arc::new((
            Mutex::new(PendingOutput {
                data: data.into(),
                reader_done: false,
    ...
    #[test]
    fn reader_completion_drains_queued_chunks_once() {
        let pending = pending("first");
        lock_pending(&pending.0).data.push_str(" second");
        drop(ReaderDone(pending.clone()));
        let mut emitted = Vec::new();
        run_shell_flusher(&pending, &fresh_shared(), |data| emitted.push(data));
        assert_eq!(emitted, vec!["first second"]);
        assert!(lock_pending(&pending.0).data.is_empty());
    }

    #[test]
    fn disposal_suppresses_buffered_data_even_when_reader_ends_at_eof() {
        let pending = pending("tail");
        let shared = fresh_shared();
        shared.lock().unwrap().disposed = true;
        drop(ReaderDone(pending.clone()));
        run_shell_flusher(&pending, &shared, |_| panic!("disposed shell emitted data"));
    }

    #[test]
    fn reader_unwind_wakes_an_idle_flusher() {
        let pending = pending("");
        let worker_pending = pending.clone();
        let (tx, rx) = std::sync::mpsc::channel();
        let worker = std::thread::spawn(move || {
            run_shell_flusher(&worker_pending, &fresh_shared(), |_| {
                panic!("empty shell emitted")
            });
            tx.send(()).unwrap();
        });
        let _ = std::panic::catch_unwind(|| {
            let _done = ReaderDone(pending.clone());
            panic!("reader unwound");
        });
        rx.recv_timeout(std::time::Duration::from_secs(2))
            .expect("flusher stranded");
        worker.join().unwrap();
    }
}
