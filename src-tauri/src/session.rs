...
fn resolve_background(input: &Value, tool: &str) -> bool {
...
fn first_nonblank_line(text: &str, n: usize) -> String {
    let line = text
        .lines()
        .map(str::trim)
        .find(|l| !l.is_empty())
...
fn fill_step_meta(s: &mut Session, agent_id: &str, seq: u32, meta: String) -> Vec<AgentEmission> {
...
fn revive_agent(s: &mut Session, agent_id: &str) -> Vec<AgentEmission> {
...
fn apply_background(s: &mut Session, agent_id: &str, background: bool) -> Vec<AgentEmission> {
    match s.agents.get_mut(agent_id) {
        Some(a) => {
...
fn user_line_text(v: &Value) -> String {
...
fn notice_is_error(text: &str) -> bool {
    text.to_lowercase()
...
fn resolve_notice_agent(s: &Session, text: &str) -> Option<String> {
...
fn apply_notice(s: &mut Session, agent_id: &str, text: &str, at: u64) -> Vec<AgentEmission> {
...
fn finalize_agents(s: &mut Session, errored: bool, at: u64) -> Vec<AgentEmission> {
...
fn apply_kill(s: &mut Session, agent_id: &str, at: u64) -> Vec<AgentEmission> {
    if !s.agents.contains_key(agent_id) {
        return Vec::new();
    }
    if let Some(a) = s.agents.get_mut(agent_id) {
...
fn activity_of(map: &HashMap<String, Session>, agent_id: &str) -> Option<Vec<AgentStep>> {
    map.values()
...
fn emit_agent_emissions(app: &AppHandle, session_id: &str, ems: Vec<AgentEmission>) {
    for e in ems {
        match e {
...
fn handle_task_notification(app: &AppHandle, session_id: &str, v: &Value) -> bool {
...
mod tests {
    ...
    fn agent_fixture(id: &str, name: &str, background: bool) -> AgentInfo {
        AgentInfo {
            id: id.into(),
            session_id: "s1".into(),
            name: name.into(),
    ...
    fn mint_agent(s: &mut Session, id: &str, name: &str, tuid: &str, background: bool) {
        s.insert_agent(agent_fixture(id, name, background));
        s.agent_by_tool.insert(tuid.into(), id.into());
    }

    fn emitted_steps(ems: &[AgentEmission]) -> Vec<AgentStep> {
        ems.iter()
    ...
    fn emitted_updates(ems: &[AgentEmission]) -> Vec<AgentInfo> {
        ems.iter()
            .filter_map(|e| match e {
