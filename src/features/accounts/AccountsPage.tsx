// multi-account FR-34..FR-36 — Settings / Accounts (Figma 21 · 140:7586, light
// 142:16488). It was the Accounts modal (redesign turn 8b, "Credential vault");
// the redesign ("Graphite & Signal") folds it into the Settings view as a page:
//
//   App / Accounts
//   Accounts                                            [Add account]
//   Each account keeps its own config — …
//   ┌ rail (260) ┐  ┌ detail ────────────────────────────────────┐
//   │ CONNECTED  │  │ Anthropic  2 logins · Claude Code CLI  [Add login]
//   │ AVAILABLE  │  │ ┌ login card · meters · sessions ┐          │
//   └────────────┘  └────────────────────────────────────────────┘
//
// Every provider in the catalog is a row, connected or not (providers.ts owns
// that as pure, vitest-covered functions). The store IS this page's source of
// truth: FR-7 makes every mutation emit account.list, which App.tsx's feed
// writes into the store; the mutations' own fresh lists only keep the cursor
// honest across a removal.
//
// Four states, scoped to the detail pane:
...
//
// Keyboard (§3): ↑/↓ move within a provider · ←/→ change provider · Enter set
// default · r rename · Del remove · a add. Esc cancels the login / rename /
// confirm; with none of those open it is left to the Settings view (close).
...
export default function AccountsPage(): JSX.Element {
  ...
  /** The header's "Add account": a Claude Code login, on Anthropic's pane (the palette's "Add account" too). */
  const addAccount = () => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') {
        // Nothing to cancel → not ours: the Settings view closes on it.
        if (!login && !renamingId && !confirmId) return;
        e.stopPropagation();
        e.preventDefault();
        if (login) closeLogin();
    ...
    // setDefault, startRename, addPrimary) read only refs/setters/
  ...
  return (
    <div className="acc-page">
        <SettingsHeader
          crumbs={['App', 'Accounts']}
          title="Accounts"
          lede="Each account keeps its own config — skills, agents and MCP servers are not shared between them."
          action={
            <Button variant="primary" disabled={busy} onClick={addAccount} title="Sign in to Claude Code with another account">
              Add account
            </Button>
          }
        />
