...
- `enabled: true`
- **Source of truth (September 2026): Figma — "Redesign · Graphite & Signal"**, file `YEY4c6AiWq1bKYuaju9qdV`, page `124:2`. It **supersedes the Claude Design mirror below**, which is kept as history for the surfaces the redesign has not re-drawn yet. Read it with the Figma MCP tools on individual frames (never the page node — it is too large): `get_design_context` for code, `get_variable_defs` for tokens, `get_metadata` to orient. Every screen has a dark frame and a light twin prefixed `L` (e.g. `01 · Session / Running` `131:185` ↔ `L01` `142:8676`); the shared components are `Components / Icons` `125:2`, `State` `125:7904`, `Button` `125:7935`, `Tab` `125:7944`, `Tag` `125:7945`, `Kbd` `125:7947`, `Switch` `125:7953`, and the shell pieces `App bar` `126:158`, `Sidebar / Sessions` `127:28`, `Sidebar / Rail` `128:68`, `Composer` `128:110`, `Transcript / Tool row` `128:122`, `Transcript / User message` `128:126`, `Session panel` `130:378`.
- **Identity "Graphite & Signal"** — a quiet graphite UI in which colour is reserved for *state*. Type is **Geist** (UI) and **Geist Mono** (code, paths, figures), self-hosted as woff2 in `src/assets/fonts/`. Surfaces are a warm-neutral graphite ladder (`--bg-canvas` → `--bg-rail` → `--bg-input` → `--bg-card` → `--bg-raised` → `--bg-selected` → `--bg-strong`) separated by **1px `--line-subtle` rules** (the 9a "no strokes" rule is retired). The one strong action per view is **ivory** (`--action-primary`, ink in light); **cyan** `--state-running` is the live thing, **violet** `--state-attention` is a session blocked on you (approvals and questions), green/red are success/danger, blue `--state-info` marks modified files. Two themes, **dark (default) and light**, switched by `data-theme` on `<html>` (moon/sun in the app bar, persisted). Token names are the Figma variable names with `/` → `-` (`bg/rail` → `--bg-rail`), documented at the top of `src/styles.css`; the pre-redesign names survive there as aliases so unconverted screens still theme. The shell is an app bar (48px) over roster · main column (session header, transcript, composer) · right-hand session panel (Changes · Plan · Activity · Context, `]`).
- *History — before September 2026:* Source: Claude Design project **"Design system extraction plan"** (`a4b15728-147c-4932-b83c-f60a5fc60db7`), mirrored locally at the repo root. The mocks are authoritative for layout, colors, glyphs, and micro-interactions. Pull a fresh copy with the `DesignSync` tool (`list_files` / `get_file`) — the local copies go stale silently.
...
- **Domains**: `app` · `session` · `conversation` · `diff` · `shell` · `agents` · `workflows` · `mcp` · `skills` · `palette` · `cli` · `project` · `standards` · `profiles` · `permissions` · `remote` · `account` · `cloud` · `extensions` · `github` · `cohorte`
...
| `cohorte-integration` *(frozen)* | Cohorte v3 runs inside Francois — `.cohorte/` detection (walk-up + git-common-dir), a per-project watcher polling `cohorte status --json` / `tail <run> --json` (dedupe by `(sequence, sub)`), all 68 Cohorte Protocol 1.0 event types bound to a typed `CohorteEvent` union on `francois://cohorte/event`, gates answered through the CLI (approve · send to fix · deny), a session-panel Cohorte tab, the `cohorte:<runId>` run view, and Settings · Cohorte (doctor, init, four Francois-side prefs). Francois never writes `.cohorte/` | projects, session-engine, sessions-sidebar, conversation-view, app-shell, notifications, command-palette, wsl-filesystem, github-page |
