...
- **CI/CD** (`.github/workflows/`):
  ...
  - `npm-publish.yml` — publishes the **`francois` npm package** (`packaging/npm/`), the primary install route: `npm i -g francois`. The package ships no binaries; its postinstall downloads the portable archives attached to a release and verifies them against sha256 digests baked into `manifest.json` at publish time. This exists because SmartScreen/Gatekeeper key off the Mark-of-the-Web / `com.apple.quarantine` attribute a *browser* attaches — a CLI-fetched binary carries none, so unsigned builds install clean. Called by `release-main.yml` after the dev matrix (`<version>-dev.<run>`, dist-tag `dev`); triggered by `release: published` for stable (dist-tag `latest`) — it waits for the draft to go live because draft assets aren't publicly downloadable. **Credentials**: prefers npm **trusted publishing** (OIDC, `id-token: write`) so no credential is stored at all; falls back to an `NPM_TOKEN` secret, needed to bootstrap the first publish. The token is never `inherit`ed — the caller passes that one secret, it is scoped to the `npm` environment, and it is bound to the publish step alone. Publishes with `--provenance`.
