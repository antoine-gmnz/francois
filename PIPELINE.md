...
| `plugin-system` | pane `[6]+` + Plugins modal + palette commands + status items: capability-sandboxed third-party extensions running in a QuickJS isolate in the core, rendering a declarative `PanelSpec` | app-shell, session-engine, conversation-view, command-palette, projects, permission-guardrails, diff-view, agents-panel, usage-bar |
