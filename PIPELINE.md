...
| `multi-provider-seam` *(blocked)* | the `SessionAdapter`/`TurnControl` trait seam between the session engine and whatever drives a turn, `ClaudeCodeAdapter` as its only implementation, the `SessionProvider`/`AccountKind` discriminators, and the `providerCapabilities()` table. Zero user-visible change | session-engine, multi-account, durable-sessions, permission-guardrails |
