...
4. The design references — the spec's `design_files` if listed, else the local mirror
   `Claude Terminal.dc.html` + `screenshots/` at the repo root (authoritative for layout, colors,
   glyphs, micro-interactions; the mock's "clyde" branding reads as "francois").
...
1. **Open the design references first** (spec `design_files`, else the local mirror
   `Claude Terminal.dc.html` + `screenshots/`) and identify the exact screens and states your
   tasks cover before writing anything.
